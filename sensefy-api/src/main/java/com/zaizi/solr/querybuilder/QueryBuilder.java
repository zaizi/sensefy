...
import com.zaizi.dto.faceting.configuration.FacetConfiguration;
...
public class QueryBuilder
{
    ...
    public static SolrQuery getSolrQuery(String query, String fields, boolean facet,
                                         ...
                                         boolean security, boolean spellcheck, boolean clustering)
    {
        if (rows == null)
        {
            rows = DEFAULT_ROWS;
        ...
        if (security)
        {
        	// TODO : Removed temporarily since Authentication still not completed
            String filterQueryACLs = SecurityQueryBuilder.getSecurityFilterQuery();
           // documentsQuery.addFilterQuery(filterQueryACLs);

        }
        String currentTimezone = TimeZone.getDefault().getID();//sensefyToken.getTimezone().getID();
        documentsQuery.set("TZ", currentTimezone);
    ...
    /**
     * takes in input a set of filter queries generate from the UI .
     * Those filter queries will be built with the appropriate tag :
     * Input:
     * ds_creator_sort:"fran", ds_creator_sort:"tico", author:"jan"
     * Expected result:
     * 1 for each field
     * fq={!tag=tag-ds_creator}ds_creator_sort:("fran" OR "tico")
     * fq={!tag=tag-author}author:("fran" OR "tico")
     *
     * @return
     */
    ...
    /**
     * Return the proper semantic query string, based on the params in input. In this way we can build semantic search
     * by entityId,entity type or entity attributes and return the query string
     *
     * @param entityId
     * @param entityType
     ...
     * 
     * public static String getQueryString( String entityId, String entityType, String entityAttribute, String
     * entityAttributeValue, SearchResponse searchResponse ) { String query; String baseQuery=""; EntityClause
     * entityClause=null; EntityTypeClause entityTypeClause; BooleanEntityQuery booleanQuery=null; if ( entityId != null
     * ) { entityClause=new EntityClause(entityId); booleanQuery=new BooleanEntityQuery( entityClause ); }
     * if(entityType!=null){ entityTypeClause=new EntityTypeClause( entityType,entityAttribute,entityAttributeValue );
     * booleanQuery=new BooleanEntityQuery( entityTypeClause ); }
     * 
