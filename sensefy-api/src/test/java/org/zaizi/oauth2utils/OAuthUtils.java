...
public class OAuthUtils
{
    ...
    public static OAuth2Details createOAuthDetails(Map config)
    {
        ...
        try
        {
            ...
            if (code >= 400)
            {
                ...
                if (isValid(accessToken))
                {
                    ...
                    if (code >= 400)
                    {
                        throw new RuntimeException("Could not access protected resource. Server returned http code: "
                                + code);
        ...
        try
        {
            ...
            if (code >= 400)
            {
                ...
                if (code >= 400)
                {
                    ...
                    if (code >= 400)
                    {
                        throw new RuntimeException("Could not retrieve access token for user: "
    ...
    public static Map handleJsonResponse(HttpResponse response)
    {
        ...
        try
        {
            oauthLoginResponse = (Map<String, String>) new org.json.simple.parser.JSONParser().parse(EntityUtils.toString(response.getEntity()));
        }
