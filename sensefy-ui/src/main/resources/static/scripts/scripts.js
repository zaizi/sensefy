...
(function() {
    ...
    controllersModule = angular.module('SearchController', ['pascalprecht.translate', 'tmh.dynamicLocale', 'LocalStorageModule']).config([
        'tmhDynamicLocaleProvider', function (tmhDynamicLocaleProvider) {
            return tmhDynamicLocaleProvider.localeLocationPattern('//code.angularjs.org/1.3.15/i18n/angular-locale_{{locale}}.js');
        }
    ]).config([
        ...
        function ($scope, $location, dataSources, $rootScope, SemanticSearchService, SensefyFacetsPerGroup, SensefySortOptions, pdf, SensefyAPIUrl, SensefyDocsPreview, localStorageService, ApiService, SensefyPreviewDoc, auth) {
            ...
            initDataSources = function (response) {
                var facet, sources, _i, _len, _ref, allOccurrence, _k, _lenK;
                sources = [];
            ...
            // clean HTML tags from search (from URL and input field)
            HTMLtagCleaner = function(richHtml){
                var regex, body, result;
                regex = /(<([^>]+)>)/ig;
                body = richHtml;
